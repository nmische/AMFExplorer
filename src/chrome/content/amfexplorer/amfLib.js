...
flex.ArrayList = flex.ArrayCollection.extend({});
...
flex.ManagedObjectProxy = flex.ObjectProxy.extend({});
		...
		for (var i = 0; i < flagsArray.length; i++)
		{
			...
			else if (i == 1)
			{
				...
				if ((flags & MESSAGE_ID_BYTES_FLAG) != 0)
				{
					...
					this.clientId = UUIDUtils.fromByteArray(messageIdBytes); 
				}
...
flex.AcknowledgeMessage = flex.AsyncMessage.extend({});
flex.AcknowledgeMessageExt = flex.AcknowledgeMessage.extend({});
flex.ErrorMessage = flex.AcknowledgeMessage.extend({});
flex.AsnycMessageExt = flex.AsyncMessage.extend({});
flex.CommandMessage = flex.AsyncMessage.extend({});
flex.CommandMessageExt = flex.CommandMessage.extend({});

UUIDUtils = Class.extend(
{
	...
	fromByteArray: function (ba) {
		...
		if (ba != null && ba.length == 16)
		{
			...
			for (var i = 0; i < 16; i++)
			{
				if (i == 4 || i == 6 || i == 8 || i == 10)
					result.push("-");
				result.push(this.UPPER_DIGITS[(ba[i] & 0xF0) >>> 4]);
				result.push(this.UPPER_DIGITS[(ba[i] & 0x0F)]);
			}
